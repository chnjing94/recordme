...
对volatile变量的写操作，在汇编层面会另外添加上一条Lock指令。该Lock指令完成两件事

1. 将当前处理器缓存行（通常为64字节）的数据写回到系统内存。
2. 这个写回内存的操作会使在其他CPU里缓存了该内存地址的数据（该内存地址上包括除volatile变量以外的数据）无效。处理器能够嗅探其他处理器访问系统内存和它们的内部缓存。如果处理器嗅探到有其他处理器修改了内存，会使当前缓存在处理器中对应的缓存行数据失效。
