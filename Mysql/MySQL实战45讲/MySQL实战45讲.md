...
因为一行数据被删除之后，不会真正地删掉该数据，只是标记为可复用，当一个页中所有数据都标记为删除时，整个页都会被标记为可重用。

## 解决方法？

开启`innodb_file_per_table`，然后使用`alter table A engine=InnoDB 命令来重建表`，5.6之后支持Online DDL来重建表。Online的意思是在重建表的过程中，增删改查请求还是可以进行。

![](./pic/13-OnlineDDL.png)

Online DDL的步骤如下：

1. 建一个临时文件，扫描表A主键的所有数据页。
2. 用数据页中表A的记录生成B+树，存到临时文件中。
3. 在生成临时文件的过程中，将所有对A的操作记录在一个日志文件（row log）中，对应图中state2。
...
2. 当前读：总是读取最新的。
   - select ... lock in share mode，加读锁，在RR下加next-key锁。
...
数据不一致；主库的数据和通过binlog备份到从库的数据不一致。
...
binlog是实现主备一致的关键。binglog传输的时机和过程如下：

![](./pic/24-主备流程图.png)

binlog三种形式之一的mixed：MySQL会判断如果用statement格式会不会在从节点上出现主备不一致，如果会，就采用row格式，如果不会，就用statement格式。